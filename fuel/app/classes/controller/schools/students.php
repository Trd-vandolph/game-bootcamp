...
class Controller_Schools_Students extends Controller_Schools
{
	...
	public function action_index()
	{
		...
		$where = [["group_id", 75],["deleted_at", 0], ["school_admitted", $this->user->id]];

		$query = Model_User::query()
			->where('group_id', '=', 75)
			->where('deleted_at', '=', 0)
			->where('school_admitted', '=', $this->user->id);
		...
		$view = View::forge("schools/students/index", $data);
	...
	public function action_paid()
	{
		...
		$view = View::forge("schools/students/paid", $data);
		$this->template->content = $view;
	}
	public function action_trial()
	{
		...
		$view = View::forge("schools/students/trial", $data);
		$this->template->content = $view;
	}
	...
	public function action_add()
	{
		...
		if(Input::post("firstname", null) != null and Security::check_token()){
			...
			if(!isset($data["error"])){
				...
				Response::redirect("/schools/students");
			}
		}
		...
		$view = View::forge("schools/students/add", $data);
		$this->template->content = $view;
	}
	...
	public function action_detail($id = 0)
	{
		...
		$view = View::forge("schools/students/detail", $data);
		$this->template->content = $view;
	}
	...
	public function action_changepassword($id = 0)
	{
		...
		$view = View::forge("schools/students/changepassword");
		$this->template->content = $view;
	}
}
