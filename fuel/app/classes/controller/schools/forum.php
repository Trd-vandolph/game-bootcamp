...
class Controller_Schools_Forum extends Controller_Schools
{
	...
	public function action_index()
	{
		...
		$data["user"] = $this->user;
		$view = View::forge("schools/forum/index", $data);
	...
	public function action_edit($id = 0)
	{
		...
		if(Input::post("title", "") != "" and Security::check_token()){
			...
			$data["forum"]->save();
			Response::redirect("/schools/forum/");
		}
		$data["user"] = $this->user;
		$view = View::forge("schools/forum/edit", $data);
		$this->template->content = $view;
	}
	...
	public function action_comment($id = 0)
	{
		...
		if(Input::post("body", "") != "" and Security::check_token()){
			...
			$data["comment"]->save();
			Response::redirect("/schools/forum/detail/{$data["comment"]->forum_id}");
		}
		$data["user"] = $this->user;
	...
	public function action_detail($id = 0)
	{
		...
		$view = View::forge("schools/forum/detail", $data);
		$this->template->content = $view;
	}
}