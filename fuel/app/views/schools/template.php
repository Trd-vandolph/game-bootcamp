...
<header>
	...
	<nav>
		...
		<ul>
			<? if($auth_status): ?>
				<li><? echo Html::anchor('/schools/profile', '<i class="fa fa-user"></i> Profile'); ?></li>
				<li><? echo Html::anchor('/schools/setting', '<i class="fa fa-cog"></i> Setting'); ?></li>
				<li><? echo Html::anchor('/schools/?logout=1', '<i class="fa fa-sign-out"></i> Logout'); ?></li>
			<? endif; ?>
		</ul>
	</nav>
</header>
	...
	<section id="user-data">
		...
		<div class="reserve">
			...
			<? if(count($donetrial) < 1  && count($pasts) < 1): ?>
				<? echo Html::anchor('/schools/lesson/add?course=-1', '<i class="fa fa-plus-circle"></i> '); ?>
			<? elseif(count($donetrial) == 1 && count($pasts) >= 0 && count($pasts) <= 23): ?>
				<? echo Html::anchor('/schools/lesson/add?course=0', '<i class="fa fa-plus-circle"></i> '); ?>
			<? elseif(count($pasts) >= 24 && count($pasts) <= 32): ?>
				<? echo Html::anchor('/schools/lesson/add?course=1', '<i class="fa fa-plus-circle"></i> '); ?>
			<? else: ?>
				<? echo Html::anchor('/schools/lesson/add', '<i class="fa fa-plus-circle"></i> '); ?>
			<? endif; ?>
		</div>
	</section>
<? endif; ?>
