	...
	<div id="main">
				...
				<? foreach($forum->comments as $comment): ?>
					<dt class="clearfix">
						...
						<div class="button-area">
							<? if($user->id == $comment->user_id) echo Html::anchor
								("teachers/contactforum/detail/{$comment->contactforum_id}/?del_id={$comment->id}",
									'<i class="fa fa-times"></i> Delete',
									["onclick" => "if(confirm('Do you want to delete it?')){}else{return false}","class" => "button gray right"]);
							?>
							<? if($user->id == $comment->user_id) echo Html::anchor
								("teachers/contactforum/comment/{$comment->id}",
								'<i class="fa fa-cog"></i> Edit',array("class" => "button green right")); ?>
						</div>
					</dt>
					<dd>
						<?= nl2br($comment->body); ?>
		...
		<section class="replay">
			<form action="" method="post" enctype="multipart/form-data">
				...
				<p class="button-area">
					<button class="button" name="action" value="confirm">Submit <i class="fa fa-chevron-right"></i></button>
				</p>
