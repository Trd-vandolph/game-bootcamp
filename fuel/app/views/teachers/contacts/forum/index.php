<div id="contents-wrap">
	<div id="main">
		...
		<div class="contact-text clearfix">
			...
			<? echo Html::anchor("teachers/contactforum/edit",
				'<i class="fa fa-plus-circle"></i> Send New Message',array('class' => 'button right'));
			?>
		</div>
		<table class="table-base contact-table" width="100%" border="0" cellpadding="0" cellspacing="0" >
			...
			<? if($news != null): ?>
				...
				<tr id="forum_<? echo $obj->id; ?>">
					...
					<td>
						<?= Html::anchor("teachers/contactforum/detail/{$obj->id}", $obj->title); ?>
						<button class="button gray right" onclick="deleteForum(<? echo $obj->id; ?>)"><i
								class="fa fa-times"></i> Delete</button>
						<?= Html::anchor("teachers/contactforum/edit/{$obj->id}", '<i class="fa fa-cog"></i> Edit', ["class" => "button green right"]); ?>
					</td>
				</tr>
				<? endforeach; ?>
			<? endif; ?>
