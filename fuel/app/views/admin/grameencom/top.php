		...
		<section>
			<div id="contents-wrap">
				<div id="main" class="new-m">
					<a name="cal"></a>
					<div id="calendar"></div>
					<p class="view-events" style="margin-right:8px; padding: 3px 10px;"><? echo Html::anchor("admin/grameencom/events/all", '<i class="fa fa-fw fa-calendar"></i> View Created Events'); ?></p>
					<h3>Students List</h3>
							...
							<? foreach($reservations as $reservation): ?>
								<tr class="course1">
									<td class="date-width">
										...
										<p class="email"><? echo Html::anchor("admin/grameencom/students/detail/{$reservation->student->id}", "View Profile"); ?></p>
									</td>
							...
							<? foreach($completed as $complete): ?>
								<tr class="course1">
									<td class="date-width">
										...
										<p class="email"><? echo Html::anchor("admin/grameencom/students/detail/{$complete->student->id}", "View Profile"); ?></p>
									</td>
...
$(function(){
	$('#calendar').clndr({
		...
		clickEvents: {
			nextMonth: function(month){
				location.href="/admin/grameencom/top/?ym=<?= date("Y-m", strtotime($ym . "-01 +1 month")); ?>#cal";
			},
			...
			click: function(target){
				...
				var path = "/admin/grameencom/reservations/?year=" + ymd[0] + "&month=" + ymd[1] + "&day=" + ymd[2];
				location.href = path;
			}
		}
	})
	...
	<? for($i = 1; $i <=  date("t", strtotime($ym . "-01")); $i++): ?>
		...
		$(".calendar-day-<?= "{$ym}-$d" ?> > .day-contents").after('<div style="color: white; background-color: rgb(126, 158, 66); " class="class-numbers grameencal"><?= ($days[$i] != 0) ? $days[$i] . ' reservation ' : ''; ?></div>');
		$(".calendar-day-<?= "{$ym}-$d" ?> > .day-contents").after('<div style="color: white; background-color: #FF9900" class="class-numbers"><?= ($events[$i] != 0) ? $events[$i] . ' event/s found ' : ''; ?></div>');
		$(".calendar-day-<?= "{$ym}-$d" ?> > .day-contents").after('<div style="color: white; background-color: #FF6600" class="class-numbers"><?= ($dayevt[$i] != 0) ? ' Whole Day Event ' : ''; ?></div>');
		...
		$(".calendar-day-<?= "{$ym}-$d" ?>").each(function(){
			<? foreach ($rss_1->entry as $item) {
					...
					if($(".calendar-day-<?= $ym . '-' .$d; ?> > #edate").text() == '<?= $no_br; ?>'){
						$(".calendar-day-<?= $ym . '-' .$d; ?> > #edate").after("<div class='gracal' style='color: white; background-color: rgb(73, 134, 231)'><?= $title; ?></div>");
		...
		$(".calendar-day-<?= "{$ym}-$d" ?>").on('mouseenter', function(){
			...
			$createLink = "/admin/grameencom/reservations/?year=".$year."&month=".$month."&day=".$day;
			$viewres = "/admin/grameencom/reservations/view/?year=".$year."&month=".$month."&day=".$day;
			$viewevents = "/admin/grameencom/events/?year=".$year."&month=".$month."&day=".$day;
			...
			if(isset($reserveinfo)){
				...
				foreach($reserveinfo as $rinfo){
					...
					if($now == $r_date && $r_count >=$r){?>
						$('.calendar-day-<?= $now; ?> .class-numbers').css('display','none');
						...
						<? if($evt>0){ ?>
							...
							$('.view-event1').css('textIndent','25px');
							$('.view-event1').css('marginTop','15px');
							$('.view-event2').css('textIndent','35px');
						<? }else{ ?>
							$(this).append('<p class="view-event2"><a href="<?=$viewres; ?>">View Reservation</a></p>');
							$(this).append('<p><a href="<?=$createLink; ?>">Create New Event</a></p>');
			...
			if(isset($eventinfo)){
				...
				foreach($eventinfo as $einfo){
					...
					$rE_id = $rE_pieces[3];
					$rE_link = "/admin/grameencom/events/edit/".$rE_id;
			...
			if(isset($info)){
				...
				foreach($info as $infos){
					...
					$id = $pieces[3];
					$link = "/admin/grameencom/events/edit/".$id;
		...
		$(".calendar-day-<?= "{$ym}-$d" ?>").on('mouseleave', function(){
			$('.calendar-day-<?= "{$ym}-$d" ?> .class-numbers').css('display','block');
