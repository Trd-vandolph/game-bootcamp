			...
			<dl>
				...
				<dt class="clearfix">
					...
					<div class="button-area">
						...
						<?= Html::anchor
							("students/forum/detail/{$comment->forum_id}/?del_id={$comment->id}", '<i class="fa fa-times"></i> Delete',
								["onclick" => "if(confirm('Do you want to delete it?')){}else{return false}","class" => "button gray right"]);
						?>
						<?= Html::anchor("students/forum/comment/{$comment->id}", '<i class="fa fa-cog"></i> Edit',array("class" => "button green right")); ?>
						<? endif; ?>
					</div>
				</dt>
				<dd>
